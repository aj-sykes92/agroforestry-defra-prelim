...
# scale
Dat_sb <- Dat_sb_full %>% even_scale(uptake)
...
The fenceline agroforestry system was implemented as occupying `r uptake * 100`% of space on otherwise unplanted fencelines. The system produced approximately `r round(fl_timb_m2, 2)` m^3^ of timber annually. Table `r tab_no` presents core results for this measure.
...
Abatement rate varied widely across the country, with typical rates of around 1 tCO~2~ ha^-1^ (Fig. `r fig_no`). The most cost-effective abatement was spread quite widely — with the majority of potentially cost-negative options focused around the south-west of England and Wales — though pasture and uplands in the north and west of the country also appeared to have some potential for cost-effective abatement (Fig. `r fig_no`).

``` {r paired_map_fig, fig.cap = "Marginal abatement cost and abatement rate for combined agroforestry measures, assuming uptake countrywide. AR is given in tonnes CO2 sequestered per hectare of agricultural land per year, MAC in 2019 GBP tonne CO~2~ sequestered^-1^."}
Dat_ag %>%
  build_paired_map()
...
Combining the measures into a marginal abatement cost curve (MACC) demonstrated some overlap between agroforestry system types, with the range of marginal abatement costs for row orchards and shelterbelts spreading this measure across fenceline planting and some intercropping/row agroforestry) (Fig. `r fig_no`).
...
```{r ag_agf_agmacc_fig, fig.height = 3, fig.cap = "Marginal abatement cost curve for combined agroforestry measures. Vertical bars represent individual crop + grid cell pairings. The dashed horizontal lines represent zero and the social cost of carbon [@Nordhaus2017]."}
Dat_ag %>%
  build_agmacc_plot() +
